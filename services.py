...
#auto update time log
import datetime as _dt

#put this on .env file
...
##This part2 is after server testing run uvicorn main:app --reload first giving authentication token to the user
...
async def get_current_user(db: _orm.Session = _fastapi.Depends(get_db), token: str = _fastapi.Depends(oauth2schema)):
    try: 
        _payload = _jwt.decode(token, JWT_SECRET, algorithms=["HS256"])
        user = db.query(_models.User).get(_payload["id"])
    except:
        raise _fastapi.HTTPException(status_code=401, detail="Invalid Email or Password")
    return _schemas.User.from_orm(user)

## part 3 create func method get lead
...
#get single lead function | helper
#1 filter by user then filter lead from user
